...
这些演示视频已上传为 GitHub `user-attachments`，因此在 README 中会直接显示可播放的视频播放器；仓库内 MP4 与 poster 继续保留在素材目录，用于审计、重新采集和后续重新上传。
...
![CodeInsights 系统架构：Electron 三进程、Bun workspace、Agent、Pipeline、Coding Runtimes 与本地状态](./assets/imgs/codeinsights-system-architecture.png)
...
![CodeInsights Pipeline v2 LangGraph 流程：Claude Explorer/Planner、Codex Developer/Reviewer/Tester/Committer、人类 Gate 与 patch-work 证据](./assets/imgs/codeinsights-pipeline-langgraph-flow.png)
...
![CodeInsights Agent Runtime 流程：Renderer、Agent 编排、session-first Runtime 选择、Claude Code、Codex、opencode、权限与 JSONL 事件存储](./assets/imgs/codeinsights-agent-runtime-flow.png)
...
![CodeInsights IPC 与 Renderer 状态流：shared 类型、main handlers、preload 白名单、stream bus、全局监听器和 Jotai 状态](./assets/imgs/codeinsights-ipc-state-flow.png)
...
![CodeInsights 本地存储与配置结构：配置根目录、JSON 配置、JSONL 记录、runtime manifest、Pipeline artifacts 与 repo patch-work](./assets/imgs/codeinsights-local-storage-framework.png)
