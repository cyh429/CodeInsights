...
- 当前本地配置没有飞书配置；阶段 14C 按用户指示不以飞书真实入口为阻塞，但飞书入口和飞书群聊 MCP 若要对外声明仍需后续在可用环境中补跑。
...
- [x] 阶段 14B：仅在 14A 通过并单独提交后评估 Pipeline Runner v2 默认开启；已重新补真实 Pipeline 深水位 UI run。
- [x] 阶段 14C：按用户指示排除飞书真实入口阻塞后，完成 Channels v2 默认开启代码侧评估；不伪造飞书入口或群聊 MCP 通过。
- [x] 所有默认化实现都必须保留显式 env 回滚能力，并保留旧 Agent 主循环、Pipeline legacy adapter、旧 Feishu bridge 和旧 session JSONL 兼容。

### 阶段 14A 任务

- [x] 实现前 check-in：确认只修改 Agent Runner v2 默认开关和必要测试/文档，不直接触碰 Pipeline / Feishu 默认值。
- [x] 设计可回滚开关语义：未设置 env 走新默认，显式关闭 env 可回到旧 Agent 主循环，显式开启 env 继续强制 Runner v2。
- [x] 修改 `agentRuntimeRunnerV2` 默认策略，并补充默认开启 / 显式关闭聚焦测试。
- [x] 复跑默认化前验证矩阵，记录基线结果。
- [x] 复跑默认化后验证矩阵，证明默认 Agent 对话走 Runner v2，显式关闭能回到旧主循环。
- [x] 更新 `tasks/todo.md`、本 checklist 和必要交接文档。
- [x] 阶段 14A 通过后单独提交，不纳入 `.DS_Store`、`improve/`、`patch-work/` 或无关文件。
...
- [x] 显式关闭 env 可回到旧 Agent 主循环。
- [x] 旧 Agent 主循环代码保留，不删除旧 retry / Watchdog / Teams / SDKMessage 兼容路径。
- [x] `agentRuntimePipelineRunnerV2` 与 `agentRuntimeChannelsV2` 继续默认关闭。
- [x] 如默认化后真实 Electron 回归失败，回滚本阶段默认开关改动，保留阶段 13 证据和旧默认行为。

### 阶段 14B 任务

- [x] 实现前 check-in：确认只修改 Pipeline Runner v2 默认开关和必要测试/文档，不直接触碰 Channels v2 默认值。
- [x] 设计可回滚开关语义：未设置 env 走 Pipeline Runner v2，显式关闭 env 可回到 Pipeline legacy adapter，显式开启 env 继续强制 Runner v2。
- [x] 修改 `agentRuntimePipelineRunnerV2` 默认策略，并补充默认开启 / 显式关闭 / 显式开启聚焦测试。
- [x] 补充模块导入隔离测试，验证 env 在导入时决定模块级 flag。
- [x] 复跑默认化前验证矩阵，记录基线结果。
- [x] 复跑默认化后验证矩阵，证明默认 Pipeline Claude 节点走 Runner v2，显式关闭能回到 legacy adapter。
- [x] 更新 `tasks/todo.md`、本 checklist、阶段 14B baseline 和必要交接文档。
- [x] 阶段 14B 通过后单独提交，不纳入 `.DS_Store`、`improve/`、`patch-work/` 或无关文件。
...
- [x] 显式关闭 env 可回到 Pipeline legacy adapter。
- [x] Pipeline legacy adapter 代码保留，不删除旧 Pipeline 执行路径。
- [x] `agentRuntimeChannelsV2` 在 14B 阶段继续默认关闭。
- [x] 如默认化后真实 Electron 回归失败，回滚本阶段默认开关改动，保留阶段 13/14A 证据和旧默认行为。

### 阶段 14C 任务

- [x] 实现前 check-in：用户明确暂不考虑飞书问题，本阶段不以飞书真实配置、飞书入口或飞书群聊 MCP 为默认化阻塞条件。
- [x] 设计可回滚开关语义：未设置 env 走 Channels v2，显式关闭 env 可回到旧 Feishu bridge 路径，显式开启 env 继续强制 Channels v2。
- [x] 修改 `agentRuntimeChannelsV2` 默认策略，并补充默认开启 / 显式关闭 / 显式开启聚焦测试。
- [x] 补充模块导入隔离测试，验证 env 在导入时决定模块级 flag。
- [x] 确认本机无飞书配置时，不补跑飞书入口或群聊 MCP，也不声明飞书真实通过。
- [x] 更新 `tasks/todo.md`、本 checklist、阶段 14C baseline 和必要交接文档。
- [x] 阶段 14C 通过后单独提交，不纳入 `.DS_Store`、`improve/`、`patch-work/` 或无关文件。
