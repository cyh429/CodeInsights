...
- 在 `@codeinsights/shared` 新增 `AgentStreamEnvelope` / `AgentRuntimeEvent`。
...
- settings 合并时只覆盖白名单 key，冲突写入 `.codeinsights-conflicts.json` 并阻断，而不是静默覆盖用户配置。
...
- 删除插件只删除 CodeInsights 管理的 snapshot，不删除用户原始目录。
...
目标：将 CodeInsights 宿主能力以 MCP 工具暴露给 Claude Code。
UI 约束：只改变后台能力暴露，不新增或重排客户端面板。

优先工具：

- `codeinsights_send_message`
...
- Claude Code 能原生看到 `mcp__codeinsights__*` 工具。
...
1. 先实现只读工具：`codeinsights_workspace_search`、`codeinsights_list_workspace_files`、`codeinsights_memory_search`。
2. 再实现低风险 side effect：`codeinsights_open_file`。
3. 最后实现写入/外发：`codeinsights_memory_append`、`codeinsights_send_channel_message`、`codeinsights_schedule_task`。
...
happyclaw 的多用户、IM-first、Docker、计费不适合 CodeInsights 当前项目原则。
