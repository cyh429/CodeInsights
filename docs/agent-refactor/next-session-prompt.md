...
- 阶段 14B Pipeline Runner v2 默认化已完成；若 git log 中未看到本阶段提交，先检查工作树并提交当前阶段成果。
...
下一步：
1. 检查 `~/.rv-insights/feishu.json` 与 `~/.rv-insights-dev/feishu.json`；若仍不存在，继续明确记录阻塞，不伪造通过。
2. 若飞书配置存在，先更新 `tasks/todo.md` 进入阶段 14C 计划，再单独补跑 `agentRuntimeChannelsV2` 飞书入口与飞书群聊 MCP。
3. 若飞书配置仍不存在，不要默认开启 `agentRuntimeChannelsV2`。
4. 不删除旧 Agent 主循环、Pipeline legacy adapter、旧 Feishu bridge 或旧 session JSONL 兼容，除非先建立新的独立阶段计划和验证矩阵。
5. 后续任何非琐碎改动继续先写 `tasks/todo.md` 计划，完成后运行验证并单独提交。
```
