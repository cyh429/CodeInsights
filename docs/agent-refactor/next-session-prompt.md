...
2. 不删除旧 Agent 主循环、Pipeline legacy adapter、旧 Feishu bridge 或旧 session JSONL 兼容，除非先建立新的独立阶段计划和验证矩阵。
3. 若需要声明飞书真实可用，再单独补飞书入口与飞书群聊 MCP 真实验证；缺配置时继续记录为真实飞书阻塞。
4. 后续任何非琐碎改动继续先写 `tasks/todo.md` 计划，完成后运行验证并单独提交。
```
