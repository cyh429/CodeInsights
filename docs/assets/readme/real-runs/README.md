...
| `codeinsights-real-run-overview.mp4` | 6 秒真实界面录屏，覆盖 Pipeline、Agent、设置与回到 Pipeline | README 顶部演示视频 |
...
当前根 README 保留真实录屏 `<video>` 播放器，并提供 MP4 文本链接作为 fallback。如果要提高 GitHub README 直播放稳定性，需要在仓库页面上传 MP4 后改成 `https://github.com/user-attachments/assets/...` 地址，并复核线上渲染。
