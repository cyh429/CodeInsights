...
- `POST /session/:id/permissions/:permissionID`：响应 permission，body 为 `{ response, remember? }`；`response` 语义来自权限 UI 的 `once | always | reject`，最终字段类型以 SDK 生成类型为准。
...
- built-in provider 是否都接受 `provider.{id}.options.apiKey = "{env:...}"`。
- OpenAI-compatible provider 使用 `@ai-sdk/openai-compatible` 还是 `@ai-sdk/openai`，取决于目标端点是 `/v1/chat/completions` 还是 `/v1/responses`。
- `baseURL`、`headers`、`models` 的 schema 与 `https://opencode.ai/config.json` 一致。
- `/provider` 和 `/config/providers` 返回的 provider/model id 是否和 `model` 字段完全一致。
- server `PUT /auth/:id` 是否会写入 `~/.local/share/opencode/auth.json`；如果会，首版不得用它承载 CodeInsights channel key。
...
- opencode config 支持 `{env:VAR}` 变量替换；local MCP `environment` 字段首选也使用 env placeholder，但仍需真实 smoke 验证该字段会被替换。
...
- CodeInsights “allow once” -> `{ response: "once", remember: false }`。
- CodeInsights “allow always/session” -> `{ response: "always", remember: true }`，但只记录为当前 session 授权。
- CodeInsights “deny” -> `{ response: "reject", remember: false }`。
- 如果 SDK 类型显示 `remember` 与 `response: "always"` 二选一，应以 SDK 类型为准，并在 adapter 中保留统一函数 `toOpencodePermissionResponse()`。
