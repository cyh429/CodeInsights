 ...
 * - 发送消息/输入状态
 *
 * 消息路由到 CodeInsights Agent，回复通过 iLink API 发送。
 */
...
import { WECHAT_IPC_CHANNELS, WECHAT_ITEM_TYPE, WECHAT_MESSAGE_TYPE, WECHAT_MESSAGE_STATE } from '@codeinsights/shared'
  ...
  async sendMessage(toUserId: string, items: WeChatMessageItem[], contextToken: string): Promise<SendMessageResponse> {
    return this.post<SendMessageResponse>('/ilink/bot/sendmessage', {
      msg: {
        ...
        to_user_id: toUserId,
        client_id: `codeinsights_${Date.now()}`,
...
class WeChatBridge {
  ...
  private async handleMessage(msg: WeChatIncomingMessage): Promise<void> {
    ...
    if (!binding) {
      await this.client.sendText(chatId, '请先在 CodeInsights 设置中选择 Agent 渠道。', contextToken)
      return
    }
