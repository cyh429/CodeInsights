 ...
 * Bun 运行时路径检测模块
 *
 * Bun 是 CodeInsights 的可选组件（不影响核心 Agent 功能，SDK 自带编译好的 claude 二进制）。
 * 仅用于：
...
import type { BunRuntimeStatus, PlatformArch } from '@codeinsights/shared'
...
export async function detectBunRuntime(): Promise<BunRuntimeStatus> {
  ...
  console.log('[Bun 检测] 未找到 Bun（可选，不影响 CodeInsights 核心功能）')
  return {
    available: false,
    path: null,
    version: null,
