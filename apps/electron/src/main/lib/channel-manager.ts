 ...
 * 数据持久化到 ~/.codeinsights/channels.json。
 */
...
import { PROVIDER_DEFAULT_URLS } from '@codeinsights/shared'
import { getFetchFn } from './proxy-fetch'
import { getEffectiveProxyUrl } from './proxy-settings-service'
import { normalizeAnthropicBaseUrl, normalizeBaseUrl } from '@codeinsights/core'
...
export async function testChannel(channelId: string): Promise<ChannelTestResult> {
  ...
  try {
    switch (channel.provider) {
      ...
      default:
        return { success: false, message: `不支持的供应商: ${channel.provider}。你可能过去使用的是 CodeInsights 商业版，请重新下载商业版覆盖安装，当前版本为开源版本。` }
    }
  } catch (error) {
