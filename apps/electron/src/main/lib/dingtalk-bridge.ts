 ...
 * - 消息路由到 CodeInsights Agent，通过 sessionWebhook 回复
 *
...
import { DINGTALK_IPC_CHANNELS } from '@codeinsights/shared'
...
class DingTalkBridge {
  ...
  private async processRobotMessage(msg: DWClientDownStream): Promise<void> {
    ...
    if (!binding) {
      await this.replyTextViaWebhook(data.sessionWebhook, '请先在 CodeInsights 设置中选择 Agent 渠道。')
      return
    }
