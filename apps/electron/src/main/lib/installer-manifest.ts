 ...
 *
 * 从 codeinsights-api 的 /api/v1/installers/manifest 接口拉取第三方安装包清单，
 * 带 5 分钟缓存和内置 fallback——断网或接口不可用时至少能拿到官方上游 URL。
 */

import type { InstallerManifest, InstallerSource } from '@codeinsights/shared'

const CODEINSIGHTS_API_BASE = 'https://api.codeinsights.cool'
const MANIFEST_URL = `${CODEINSIGHTS_API_BASE}/api/v1/installers/manifest`
  ...
  try {
    const response = await fetch(MANIFEST_URL, {
      headers: {
        ...
        'User-Agent': 'CodeInsights-Desktop-App',
      },
    })
