...
import type { FeishuTestResult, FeishuChatBinding, FeishuBotConfig, FeishuBotBridgeState } from '@codeinsights/shared'
...
function FeishuCliSection(): React.ReactElement {
  ...
  return (
    <SettingsSection
      title="配置飞书 CLI"
      description="飞书官方开源的命令行工具，配置后 CodeInsights Agent 将可以直接读消息、查日历、写文档、建多维表格、发邮件，把任务真正落到飞书里完成。"
    >
      <SettingsCard divided={false}>
        <div className="px-4 py-4 space-y-2 text-sm text-muted-foreground">
          <p className="text-xs">复制配置提示词，并前往飞书Bot日常绑定的<strong>工作区</strong>，创建新的 CodeInsights Agent 对话并发送即可让 CodeInsights 协助完成配置。</p>
          <button
            type="button"
            className="flex items-center gap-1.5 text-xs text-muted-foreground hover:text-foreground transition-colors cursor-pointer"
            onClick={() => setExpanded(!expanded)}
...
function FeishuBindingsTab(): React.ReactElement {
  ...
  return (
    <div className="space-y-8">
      <SettingsSection
        title="绑定管理"
        description="查看和管理飞书聊天与 CodeInsights 工作区/会话的绑定关系"
        action={
          <Button
            size="sm"
            variant="outline"
        ...
        <SettingsCard divided={false}>
          <div className="px-4 py-4 space-y-5 text-sm">
            ...
            <div className="pl-7 p-3 rounded-lg bg-amber-500/10 text-amber-700 dark:text-amber-400 text-xs">
              ...
              即可通过飞书向 CodeInsights Agent 发送指令。
            </div>
          </div>
        </SettingsCard>
      </SettingsSection>
