 ...
 * - 监听 codeinsights:clear-context 和 codeinsights:focus-input 自定义事件
 * - 卡片式容器样式
 */
...
export function ChatInput({ conversationId, streaming, pendingAttachments, onSetPendingAttachments, onSend, onStop, onClearContext }: ChatInputProps): React.ReactElement {
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('codeinsights:clear-context', handler)
    return () => window.removeEventListener('codeinsights:clear-context', handler)
  }, [onClearContext])
