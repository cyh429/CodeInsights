...
import type { AgentMessage, AgentEventUsage, RetryAttempt, SDKMessage } from '@codeinsights/shared'
...
export function AgentMessages({ sessionId, sessionModelId, messages, messagesLoaded, persistedSDKMessages, streaming, streamState, liveMessages, sessionPath, attachedDirs, stoppedByUser, onRetry, onRetryInNewSession, onFork, onRewind, onCompact }: AgentMessagesProps): React.ReactElement {
  ...
  return (
    ...
    <Conversation resize={ready && !transitioning ? 'smooth' : 'instant'} className={cn('agent-conversation-canvas agent-logstream-canvas h-full', ready ? 'opacity-100 transition-opacity duration-200' : 'opacity-0')}>
