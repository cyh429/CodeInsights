...
import type { PlatformArch, BunDownloadInfo } from '@codeinsights/shared'
...
async function downloadFile(url: string, destPath: string): Promise<void> {
  ...
  const response = await fetch(url, {
    headers: {
      'User-Agent': 'CodeInsights-Build-Script/1.0',
    },
    redirect: 'follow',
  })
...
async function getBunVersion(): Promise<string> {
  ...
  const version = pkg.codeinsights?.bun?.version
  if (!version) {
    throw new Error('package.json 中未配置 codeinsights.bun.version')
  }
...
async function main(): Promise<void> {
  ...
  console.log('CodeInsights Bun 二进制下载脚本')
  console.log('='.repeat(60))
